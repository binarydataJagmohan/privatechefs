...
export default function TopRatedChef() {
  ...
  const handleCheckboxChange = (e: any, chefId: number) => {
    if (e.target.checked) {
      ...
      setTopRatedChef((prevTopRatedChefs) => prevTopRatedChefs.filter((id) => id !== chefId));
    }
  };
        ...
        <div className="table-box " id="villa_table">
          <form onSubmit={handleTopRatedChef}>
              ...
              <tbody>
                {chefdata.map((data: any, index) => (
                  <tr key={index}>
                    ...
                    <td>{data.email}</td>
                    <td>
                      <input type="checkbox" name="top_rated" id="top-rated" onChange={(e) => handleCheckboxChange(e, data.id)} checked={topratedchef.includes(data.id)} />
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            <div className="text-right">
              <button className="table-btn">Update Top Rated Chef Information</button>
            </div>
          </form>
        </div>
      </div>
