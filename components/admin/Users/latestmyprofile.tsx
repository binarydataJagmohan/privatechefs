  ...
  return (
    <>
      ...
      <div className="row user-class pt-5">
        ...
        <div className="col-sm-7">
          ...
          <ul style={{ listStyle: "none", paddingBottom: "20px", display: "inline-flex" }}>
            {getUsers?.role ? (
              <li style={{ paddingRight: "10px", textTransform: "capitalize" }}>
                <i className="fa fa-address-card" style={{ paddingRight: "10px", fontSize: "14px", color: "#ff4e00d1" }}></i>
                {getUsers.role}
              </li>
            ) : (
              ""
            ...
            {getUsers?.currency ? (
              <li style={{ paddingRight: "10px" }}>
                <i className="fa fa-sack-dollar" style={{ paddingRight: "10px", fontSize: "14px", color: "#ff4e00d1" }}></i>
                {getUsers.currency}
              </li>
            ) : (
              ""
            ...
            {getUsers?.address ? (
              <li style={{ paddingRight: "10px" }}>
                <i className="fa fa-location-dot" style={{ paddingRight: "10px", fontSize: "14px", color: "#ff4e00d1" }}></i>
                {getUsers.address}
              </li>
            ) : (
              ""
            ...
            {getUsers?.city ? (
              <li style={{ paddingRight: "10px" }}>
                <i className="fa fa-city" style={{ paddingRight: "10px", fontSize: "14px", color: "#ff4e00d1" }}></i>
                {getUsers.city}
              </li>
            ) : (
              ""
            ...
            {getUsers?.country ? (
              <li style={{ paddingRight: "10px" }}>
                <i className="fa fa-flag" style={{ paddingRight: "10px", fontSize: "14px", color: "#ff4e00d1" }}></i>
                {getUsers.country}
              </li>
            ) : (
              ""
            ...
            {getUsers?.post_code ? (
              <li style={{ paddingRight: "10px" }}>
                <i className="fa fa-map-pin" style={{ paddingRight: "10px", fontSize: "14px", color: "#ff4e00d1" }}></i>
                {getUsers.post_code}
              </li>
            ) : (
              ""
          ...
          {getUsers?.phone ? (
            <p style={{ paddingBottom: "10px" }}>
              <i className="fa fa-phone" style={{ paddingRight: "10px", fontSize: "14px", color: "#ff4e00d1" }}></i>
              <span id="book-user">Phone</span> : {getUsers.phone}
            </p>
          ) : null}
          {getUsers?.email ? (
            <p style={{ paddingBottom: "10px" }}>
              <i className="fa fa-envelope" style={{ paddingRight: "10px", fontSize: "14px", color: "#ff4e00d1" }}></i>
              <span id="book-user">Email</span> : {getUsers.email}
            </p>
          ) : null}
          {getUsers?.birthday ? (
            <p style={{ paddingBottom: "10px" }}>
              <i className="fa fa-birthday-cake" style={{ paddingRight: "10px", fontSize: "14px", color: "#ff4e00d1" }}></i>
              <span id="book-user">Birthday</span> : {getUsers.birthday}
            </p>
          ) : null}
          {getUsers?.timezone ? (
            <p style={{ paddingBottom: "10px" }}>
              <i className="fa fa-clock" style={{ paddingRight: "10px", fontSize: "14px", color: "#ff4e00d1" }}></i>
        ...
        <div className="col-sm-3">
          ...
          {getUsers?.company_name ? (
            <p style={{ paddingBottom: "10px" }}>
              <i className="fa fa-building" style={{ paddingRight: "10px", fontSize: "14px", color: "#ff4e00d1" }}></i>
        ...
        <div className="tab">
          <button className="tablinks" onClick={(event) => openCity(event, "Booking")}>
            Booking
          </button>
          <button className="tablinks" onClick={(event) => openCity(event, "Payments")}>
            Payments
          </button>
          <button className="tablinks" onClick={(event) => openCity(event, "Notifications")}>
            Notifications
          </button>
          {/* <button className="tablinks" onClick={(event) =>openCity(event, 'Activity')}>Activity</button> */}
          <button className="tablinks" onClick={(event) => openCity(event, "Edit profile")}>
            Edit profile
          </button>
          <button className="tablinks" onClick={(event) => openCity(event, "Profile status")}>
            Profile status
          </button>
        </div>
      </div>
        ...
        <div className="table-box concierge">
          {loading == false ? (
            ...
            <div className="text-center">
              <img src={`${process.env.NEXT_PUBLIC_BASE_URL}/images/chef_loading_img_3.gif`} width={50} />
              <p>Loading</p>
            </div>
          )}
        </div>
      ...
      <div id="Payments" className="tabcontent">
        ...
        <Pagination items={totalBookingPayment.length} currentPage={currentPage} pageSize={pageSize} onPageChange={onPageChange} />
      </div>
          ...
          <div className="row">
            <div className="col-lg-3 col-md-12">
              <div className="user-img" id="admin-profile">
                {getUsers?.pic ? (
                  <img src={image && typeof image !== "string" ? URL.createObjectURL(image) : getUsers.pic ? process.env.NEXT_PUBLIC_IMAGE_URL + "/images/chef/users/" + getUsers.pic : ""} style={{ objectFit: "contain" }} alt="" />
                ) : (
                  <img src={image && typeof image !== "string" ? URL.createObjectURL(image) : process.env.NEXT_PUBLIC_IMAGE_URL + "/images/users.jpg"} style={{ objectFit: "contain" }} alt="" />
                )}
                <label>
                  {" "}
              ...
              <div className="all-form tab-m-0 pt-0 right-left-spacing">
                <div className="row">
                  ...
                  <div className="text-right mt-5">
                    <button className="table-btn">Update User Profile Information</button>
                  </div>
                </div>
              </div>
            </div>
      ...
      <div id="Profile status" className="tabcontent">
        <div className="row">
          <div className="col-sm-6">
            ...
            <div className="text-center">
              <button className="table-btn" onClick={(e: any) => userAccountDeactivate(e, getUsers?.id)}>
                Deactivate Account
              </button>
            </div>
          </div>
          <div className="col-sm-6">
            ...
            <div className="text-center">
              <button className="table-btn" onClick={(e: any) => userAccountDelete(e, getUsers?.id)}>
                Delete Account
              </button>
            </div>
          </div>
