...
import {
  getConciergeMessageData,
  getClickConciergeChefUserChatData,
  ContactByConciergeToUserAndChef,
  ContactByConciergeToUserAndChefWithShareFile,
  getAllConciergeUserData,
  SendMessageToUserByConcierge,
  CreateGroupChatByConcierge,
  deleteChatMessage,
} from "../../../lib/concierge";
...
export default function Booking(props: any) {
  ...
  useEffect(() => {
    ...
    if (data == 1) {
      ...
      const fetchData = async () => {
        ...
        getClickConciergeChefUserChatData(data)
          .then((res) => {
            if (res.status == true) {
  ...
  const fetchUserMessageDetails = async (id: any) => {
    try {
      ...
      const res = await getConciergeMessageData(data);
      if (res.status == true) {
        setActiveChat(0);
        setAdminChatSiderBar(res.userchatsider);
        setAdminChatMessage(res.userchatdata);
  ...
  const fetchAllUserData = async (id: any) => {
    try {
      const res = await getAllConciergeUserData(id);
      if (res.status) {
        console.log(res.data);

        var fileter = setUserData(res.data);
      } else {
        toast.error(res.message, {
          position: toast.POSITION.TOP_RIGHT,
        });
  ...
  const imageChange = async (e: any) => {
    ...
    ContactByConciergeToUserAndChefWithShareFile(formData)
      .then((res) => {
        if (res.status === true) {
          setMessage("");
          fetchUserMessageDetails(currentUserData.id);
  ...
  const handlMessageSubmit = (event: any) => {
    ...
    if (Object.keys(newErrors).length === 0) {
      ...
      SendMessageToUserByConcierge(data)
        .then((res) => {
          if (res.status == true) {
            setSearchInputValue("");
            setModalConfirmTwo(false);
  ...
  const handlGroupSubmit = (event: any) => {
    ...
    if (Object.keys(errors).length === 0) {
      ...
      CreateGroupChatByConcierge(data, Uploadimage[0])
        .then((res) => {
          if (res.status == true) {
            setModalConfirm(false);
            fetchUserMessageDetails(currentUserData.id);
                            ...
                            {adminuserchefmesage.length > 0 ? (
                              adminuserchefmesage.map((message, index) => (
                                <React.Fragment key={index}>
                                  {(message.sender_role == "user" || message.sender_role == "chef" || message.sender_role == "admin") && message.bookig_send_by == null && (
                                    <li className="reply">
                                      {message.sender_pic == null ? (
                                        <img src={process.env.NEXT_PUBLIC_IMAGE_URL + "/images/users.jpg"} alt="chats-user" />
                                      ) : (
