...
import { getConciergeCalenderBookings } from '../../../lib/concierge';
...
export default function CalendarComponent() {
    ...
    useEffect(() => {
        const data = isPageVisibleToRole("concierge-calender");
        if (data == 2) {
    ...
    const fetchBookingAdminDetails = async () => {
        try {
            ...
            const res = await getConciergeCalenderBookings(userId.id);
            if (res.status) {
